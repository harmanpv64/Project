...
def add():
    ...
    lb = Label(top, text='ADD NEW NOTES', width=40).grid(row=0,column=1)
    e1 = Entry(top,width=45,textvariable=p).grid(row=0,column=1)
    b1=Label(top,text='Add Name',width=15,fg='black').grid(row=0,column=0)
    b2 =Label(top, text='Add Priority', width=15,fg='black').grid(row=1, column=0)#proority
    e2 = Entry(top, width=45,textvariable=prio).grid(row=1,column=1)
    lb=Label(top,text='Add Content',width=40).grid(row=2,column=1)
    ...
    c=Button(top, text='QUIT', width=15, height=2, bg='red',command=top.destroy)
    ...
    def save():
        ...
        conn.commit()
        top.destroy()
    br = Button(top, text='SAVE', width=15, height=2,bg='green',command=save).grid(row=4, column=0)
...
lbl=Label(frame3,text='Search Notes',height=3,width=50,).grid(row=1,column=0)
v=StringVar()
e=Entry(frame2,width=45,textvariable=v).grid(row=2,column=0)
lb1=Label(master,text='-- Notes --',height=3,width=50).grid(row=3,column=0)
...
def Up():
    ...
    def save():
        ...
        conn.commit()
        liste()
    br = Button(RIGHT, text='SAVE', width=15, height=2, bg='green', fg='orange', command=save).grid(row=4, column=0)
    liste()
def redd():
    del_id = myLis.get(myLis.curselection())
    l = list(del_id.split(","))
...
def sot():
    ...
    for row in cursor:
        myLis.insert(END, str(row))
def butt():
    b = Button(frame5, text='Update', bg='blue', fg='white', command=Up).grid  (row=7,column=3)
    c = Button(frame5, text='Delete', bg='blue', fg='white', command=dele).grid(row=5,column=3)
    d = Button(frame5, text=' Read ', bg='blue', fg='white', command=redd).grid(row=3,column=3)
    e = Button(frame5, text=' Sort ', bg='blue', fg='white', command=sot).grid (row=1,column=3)
